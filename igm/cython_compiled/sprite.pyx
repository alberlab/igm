...
def compute_gyration_radius(crd, cluster, index, copy_index):
    ...
    segments we should check all the possible combinations.
    To make it faster, we group the segments by chromosome.
    ...
    instead of all the combinations of segments. While segments
    can be hundreds, groups are surely less than the number
    of chromosomes.
    We thus arbitrarily select a representative segment from each group,
